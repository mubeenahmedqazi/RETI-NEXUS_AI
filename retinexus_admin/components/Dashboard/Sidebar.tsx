...
function NavRow({ item, isActive, isSidebarOpen, delay }: { item: NavItem; isActive: boolean; isSidebarOpen: boolean; delay: number }) {
  return (
      ...
      <div className="flex items-center justify-center py-5 border-b relative" style={{ borderColor: 'var(--border)' }}>
        <Link href="/dashboard" className="flex items-center justify-center">
          <motion.div whileHover={{ scale: 1.04 }} transition={{ duration: 0.3 }}>
            {isOpen ? <Logo size={48} withWordmark withTagline /> : <Logo size={44} />}
          </motion.div>
        </Link>
      </div>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between px-1 py-1">
              <div className="min-w-0">
                ...
                <p className="text-sm font-medium truncate" style={{ color: 'var(--foreground)' }}>{adminName}</p>
              </div>
              <ThemeToggle />
            </div>
