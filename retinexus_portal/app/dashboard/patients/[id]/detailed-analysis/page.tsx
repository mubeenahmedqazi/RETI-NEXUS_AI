...
// Overall-status badge for the LLM-scored Longitudinal Tracking History summary.
...
export default function DetailedAnalysisPage() {
  ...
  const dashboardBiomarkers = (context.report.biomarkers || []).filter(
    (b) => !EXCLUDED_FROM_BIOMARKER_DASHBOARD.includes(b.name?.toLowerCase().trim())
  );
  const lesionCounts = context.report.lesionCounts;
  const reportImages = context.report.images || {};
          ...
          {!result ? (
            ...
            >
              <ClinicalReportHeader
                reportId={`DA-${patientId.slice(-8).toUpperCase()}`}
              ...
              <div className="surface rounded-2xl p-6 clinical-section">
                <div className="flex items-center justify-between flex-wrap gap-3">
                  <div>
                    ...
                    <h2 className="text-xl font-bold" style={{ color: 'var(--foreground)' }}>{selectedTest}</h2>
              ...
              {/* ── Page 1: Biomarker Dashboard + Lesion Detection (this patient's screening data) ── */}
              ...
              <div className="surface rounded-2xl p-6 clinical-section">
                ...
                {lesionCounts ? (
                  ...
                  <p className="text-sm" style={{ color: 'var(--muted-foreground)' }}>No lesion data available</p>
                )}
              </div>

              {/* ── Page 2: output images, forced onto their own page (matches the screening report) ── */}
              ...
              {/* ── Page 3: detailed summary — findings, analysis, per-organ correlation, red flags, recommendations ── */}
              <div className="pdf-summary-page space-y-5">
                ...
                {(context.report.predictedRisk?.oneYear || context.report.predictedRisk?.fiveYear) && (
                  <div className="surface rounded-2xl p-6 clinical-section">
                    <h3 className="text-sm font-semibold flex items-center gap-2 mb-3" style={{ color: 'var(--foreground)' }}>
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      {context.report.predictedRisk?.oneYear && (
                      ...
                      {context.report.predictedRisk?.fiveYear && (
                        <div className="pl-4 border-l-2" style={{ borderColor: 'var(--brand-accent)' }}>
                ...
                {/* Longitudinal Tracking History — LLM interpretation only (status + a
                    4-sentence narrative synthesizing every previous report against the
                    current one). The raw per-visit list lives separately, below, in
                    Previous Reports. Part of the printed report, unlike the live chart
                    widget above (no-print). */}
                <div className="surface rounded-2xl p-6 clinical-section">
                  <h3 className="text-sm font-semibold flex items-center gap-2 mb-3" style={{ color: 'var(--foreground)' }}>
                  ...
                  {!trackingStatusLoading && !trackingStatus && (
                    <p className="text-sm" style={{ color: 'var(--muted-foreground)' }}>No patients history</p>
                  )}
                </div>
              ...
              {/* Actions — same position/style as the screening report's bottom action row */}
